#heavily modified, not commited to git, should have created a branch
...
# Download URL for Terraform
TERRAFORM_DOWNLOAD_URL = 'https://s3.ap-northeast-2.amazonaws.com/lab-korea-20181029/terraform'

# Paths where Terraform should be installed
TERRAFORM_DIR = os.path.join('/tmp')
...
def install_terraform():
    ...
    if os.path.exists(TERRAFORM_PATH):
        return
    #print(TERRAFORM_DOWNLOAD_URL)

    urllib.urlretrieve(TERRAFORM_DOWNLOAD_URL, '/tmp/terraform')
    os.system('chmod +x /tmp/terraform')
...
def apply_terraform_plan(s3_bucket, path):
    ...
    #print(os.system('/tmp/terraform apply /tmp/terraform.plan'))
